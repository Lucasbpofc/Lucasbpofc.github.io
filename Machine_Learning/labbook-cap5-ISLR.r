##########################################################################
#####           Autor: professor Luiz Alexandre Peternelli           #####
#####           peternelli@ufv.br                                    #####
#####                                                                #####
##########################################################################

#chapter 5 from textbook - with adaptations
...
#Combina????o - nesse caso sem reposi????o
...
attach (Auto)
mean((mpg - predict (lm.fit ,Auto))[-train ]^2) # apenas para teste. (mserro_teste)
...
library (boot)
glm.fit <- glm(mpg ~ horsepower ,data = Auto) #por ser modelo linear da pra usar, faz a valida????o cruzada automaticamente
...
# This function returns, or outputs, an estimate for ?? based on applying
# (5.7) to the observations indexed by the argument index. For instance, the
# following command tells R to estimate ?? using all 100 observations.
...
#J?? faz o bootsprep aumotamticamente.
boot(Portfolio , alpha.fn, R=1000)

# The final output shows that using the original data, ???? = 0.5758, and that
# the bootstrap estimate for SE(????) is 0.0886.
...
# estimates for ??0 and ??1, the intercept and slope terms for the linear regression
...
# using the formulas for SE( ?? ??0) and SE( ?? ??1) described in Section 3.1.2.
...
# of ??0 and ??1 on the entire data set using the usual linear regression
...
# This indicates that the bootstrap estimate for SE( ?? ??0) is 0.86, and that
# the bootstrap estimate for SE( ?? ??1) is 0.0074. As discussed in Section 3.1.2,
...
# The standard error estimates for ?? ??0 and ?? ??1 obtained using the formulas
...
# the standard estimates of SE( ?? ??0), SE( ?? ??1) and SE( ?? ??2).
